...
meta <- read_excel("data/pilot/7 days/06062023 salbutamol pilot fish metadata.xlsx",
  ...
  mutate(fish_id = as.character(fish_id),
         treatment = factor(treatment, levels = c("untreated",
                                                  ...
                                                  "100 µM Salbutamol")),
         sex = as.factor(sex),
         start_time = factor(`start time`,
                             levels = c()),
         genotype = factor(genotype, levels = c("het", "hom"))

  )
# add in a behav batch colname
meta <-
  meta$`start time` %>%
  unique %>%
  as.data.frame() %>%
  set_colnames("start time") %>%
  mutate(behavBatch = rownames(.)) %>%
  left_join(meta) %>%
  as_tibble

file_list <- list.files(path = here("data/pilot/7 days/raw_data/zones"),
                        pattern = "*.csv",
                        full.names = TRUE,
                        recursive = TRUE)
...
subject2fish <- meta %>%
  dplyr::select(fish_id, `Subject Identification` = behavBatch) %>%
...
write.csv(df, "data/pilot/7 days/processed_data/tidy_data.csv")
...
write.csv(df_a, "data/pilot/7 days/processed_data/time_in_zone.csv")
...
## STEP 7 - Final Output
write.csv(final_data, "data/pilot/7 days/processed_data/final_output.csv")
